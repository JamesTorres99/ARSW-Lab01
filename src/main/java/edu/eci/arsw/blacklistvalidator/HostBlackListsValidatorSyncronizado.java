...
public class HostBlackListsValidatorSyncronizado extends HostBlackListsValidator {
    ...
    public List<Integer> checkHost(String ipaddress, int N){
        ...
        HostBlackListsValidatorThread[] threads = new HostBlackListsValidatorThreadEficiente[N];
        int seccion = serverCount / nThreads;
        
        Monitor monitor = new Monitor();

        for (int i=0; i<N; i++) {
        	threads[i] = new HostBlackListsValidatorThreadEficiente(i*seccion, Math.min((i+1)*seccion, serverCount-1), ipaddress, monitor);
        	threads[i].start();
        }
